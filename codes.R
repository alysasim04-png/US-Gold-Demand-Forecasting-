...
autoplot(y) + 
  ...
  autolayer(STLF, series = "STLF", PI = FALSE) + 
  autolayer(Combination, series = "Combination") + 
  xlab("Year") + ylab("Gold Demand (in Tonnes)") + 
  ggtitle("Ensemble Forecast Comparison on Gold Demand")

# Part B: Bootstrapping and Bagging
baggedfc <- baggedModel(
  y = train_set,
  bootstrapped_series = bld.mbb.bootstrap(train_set, 20),
  fn = ets
) %>%
  forecast(h = h)
...
y <- data_ts[, "Gold_Demand_Tonnes"]
autoplot(y) + 
  autolayer(baggedfc, series = "Bagging ETS", PI = F) + 
  xlab("Year") + ylab("Gold Demand (in Tonnes)") + 
  ggtitle("Comparison against actual data")

# -------------------------------------------------------------------------------------

# Section 4: Model Evaluation & Selection

# Part A: Model from Regression
accuracy(fit)
...
e_autoarima <- tsCV(
  y = y,
  forecastfunction = function(y, h, xreg, newxreg) {
...
# Part B: Model from Ensemble 
accuracy(ETS, test_set)
accuracy(ARIMA, test_set)
accuracy(STLF, test_set)
accuracy(Combination, test_set)

y <- data_ts[, "Gold_Demand_Tonnes"]
h <- 12

e_ets <- tsCV(y, forecastfunction = function(y,h) forecast(ets(y), h = h), h =h)
e_arima <- tsCV(y, forecastfunction = function(y, h) forecast(auto.arima(y, lambda = 0), h = h), h = h)
e_stlf <- tsCV(y, forecastfunction = function(y, h) stlf(y, lambda = 0, h = h), h = h)
e_ensemble <- (e_ets + e_arima + e_stlf)/3

rmse_ensemble <- sqrt(colMeans(e_ensemble^2, na.rm = TRUE))
rmse_ensemble

# Part C: Model from Bootstrapping and Bagging 
accuracy(baggedfc, test_set)

bagged_forecast_fn <- function(y, h) {
  fit <- baggedETS(y, B = 20)
  fc <- forecast(fit, h = h)
...
e_bagged <- tsCV(
  y <- train_set,
  forecastfunction - bagged_forecast_fn,
  h = 12
)

rmse_bagged <- sqrt(colMeans(e_bagged^2, na.rm = TRUE))
rmse_bagged
